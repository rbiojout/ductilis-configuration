...
if [[ ! $DUCTILIS_RELEASE ]]; then
    echo "You must define DUCTILIS_RELEASE"
    exit
fi
...
echo "Installing release '$DUCTILIS_RELEASE'"
...
for var in ${VERSION_VARS[@]}; do
    ...
    eval override=\${$ENV_VAR-\$DUCTILIS_RELEASE}
    if [ -n "$override" ]; then
...
CONFIGURATION_VERSION=${CONFIGURATION_VERSION-$DUCTILIS_RELEASE}
